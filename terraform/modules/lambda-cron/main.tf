...
resource "aws_cloudwatch_event_rule" "every_five_minutes" {
  name                = "runcmd-lambda-cron-every-five-minutes-${var.ws_name}"
  description         = "Fires every five minutes"
  schedule_expression = "rate(5 minutes)"
}

resource "aws_cloudwatch_event_target" "runcmd_lambda_cron_every_five_minutes" {
  rule      = "${aws_cloudwatch_event_rule.every_five_minutes.name}"
  target_id = "runcmd_lambda_cron"
  arn       = "${aws_lambda_function.runcmd_lambda_cron.arn}"
}

resource "aws_lambda_permission" "allow_cloudwatch_to_call_runcmd_lambda_cron" {
  statement_id  = "AllowExecutionFromCloudWatch"
  action        = "lambda:InvokeFunction"
  function_name = "${aws_lambda_function.runcmd_lambda_cron.function_name}"
...
resource "aws_lambda_function" "runcmd_lambda_cron" {
  ...
  function_name    = "runcmd-lambda-cron-${var.ws_name}"
